#' Standardfehler bei einem Unternehmenseffekt
#'
...
#' @param Anteil_u Anteil des Unternehmenseffekts an der gesamten Varianz der Fehlervariable
...
#' @importFrom plm plm
#' @export
Unternehmenseffekt <- function(N = 500, T = 10, Anzahl_Regressoren = 1, Anz_Sim = 100, Anteil_u = 0.25){
  beta <- 1

  # Varianz und Korrelation der erklärenden Variablen und der Fehlervariable

  for(i in 1:Anzahl_Regressoren){
    assign(paste0("AnteilVarIndEffektX",i),
           as.double(readline(prompt = paste("Geben Sie den Anteil der Varianz des Unternehmenseffekts an der gesamten Varianz von", paste0("X",i) ,"an ")))
    )
  }
  for(r in 1:Anzahl_Regressoren){
    assign(paste0("Var_X", r), 1^2, envir = .GlobalEnv)
    assign("x",get(paste0("AnteilVarIndEffektX",r)))
    assign("y",get(paste0("Var_X",r)))
    assign(paste0("Var_mu", r), x*y, envir = .GlobalEnv)
  }
  Var_eta <- 2^2
  ...
  for(D in 1:Anz_Sim){
    ...
    for(r in 1:Anzahl_Regressoren){
      ...
      nu <- rnorm(N*T, mean = 0, sd = sqrt(VarX-Varmu))

      assign(paste0("X", r), mu_neu+nu)
    }
