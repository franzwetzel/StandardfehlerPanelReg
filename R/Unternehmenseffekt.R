...
#'
#' Die Funktion berechnet die geschätzten und wahren Standardfehler bei einem Unternehmenseffekt.
...
Unternehmenseffekt <- function(N = 500, T = 10, Anzahl_Regressoren = 1,
  ...
  for(i in 1:Anzahl_Regressoren){
    ...
    colnames(x) <- c("OLS", "Fama-MacBeth", "Cluster", "Newey-West",
                     "wahre Stdfehler OLS", "wahre Stdfehler Fama")
  ...
  for(D in 1:Anz_Sim){
    ...
    for(r in 1:Anzahl_Regressoren){
      ...
      nu <- stats::rnorm(N*T, mean = 0, sd = sqrt(VarX-Varmu))
